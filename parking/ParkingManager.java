...
public class ParkingManager {
	...
	public ParkingManager(ParkingFloors floor,Coupen coupen)
	{
		payment=new Payment(coupen);
		this.floor=floor;
	}
	...
	public void alloteSpace(Customer customer, Reservation t,int duration) 
	{
		...
		Ticket ticket=new Ticket(customer,alloted.getName());
		ticket.print();
		floor.getTransaction().get(alloted.getType()).add(payment.calculateAmmount(ticket, duration));
	...
	public void cancelReservation(String name) 
	{
		...
		floor.getTransaction().get(thread.ticket.getVehicleType()).add(payment.calculateRefund(thread.getTicket()));
